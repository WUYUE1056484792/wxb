...
<div id="page-content" class="clearfix">
  ...
  <div class="row-fluid">
	...
	<div class="row-fluid">
			...
			<!-- 检索  -->
			<form action="listShopPage" method="post" name="userForm" id="userForm">
			<table>
				<tr>
					<td>
					...
					<td style="vertical-align:top;">
					 	<select class="chzn-select" name="shop_validate_status_id"  data-placeholder="选择店铺状态" style="vertical-align:top;width: 120px;">
						<option value=""></option>
						<option value="">全部</option>
						<c:forEach items="${shopTypes}" var="st">
							<option value="${st.dictionary_id}">${st.dictionary_name}</option>
						</c:forEach>
					  	</select>
					</td>
					<c:if test="${QX.cha == 1 }">
					...
					<c:when test="${not empty shopList}">
						...
						<c:forEach items="${shopList}" var="shop" varStatus="vs">
							...
							<tr>
								...
								<td>${shop.shop_validate_status_name }</td>
								<td>${fn:substring(shop.shop_creation_date, 0, 19)}</td>
								<td style="width: 60px;">
									<div class='hidden-phone visible-desktop btn-group'>
											...
											<c:if test="${user.USERNAME != 'admin'}"><a class='btn btn-mini btn-info' title="修改店铺" onclick="editUser('${shop.shop_id}');"><i class='icon-edit'></i></a></c:if>
	...
	<!-- PAGE CONTENT ENDS HERE -->
		...
		//修改
		function editUser(shop_id){
			 top.jzts();
			 var diag = new top.Dialog();
			 diag.Drag=true;
			 diag.Title ="资料";
			 diag.URL = '<%=basePath%>findShopInfo?shop_id='+shop_id;
			 diag.Width = 440;
			 diag.Height = 600;
			 diag.CancelEvent = function(){ //关闭事件
				 if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
